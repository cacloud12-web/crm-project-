...
class CrmUatResetCommand extends Command
{
    protected $signature = 'crm:reset-uat
                            {--force : Required to delete all transactional CRM data}';

    protected $description = 'Remove all transactional/test CRM data for fresh end-to-end testing (preserves users, roles, and master configuration)';

    public function handle(UatResetService $resetService): int
    {
        if (! $this->option('force')) {
            $this->error('This command permanently deletes all leads, assignments, campaigns, demos, sales, and related transactional data.');
            $this->line('Users, roles, and master configuration are preserved. All employee profiles are removed.');
            $this->line('Re-run with --force to continue.');

            return self::FAILURE;
        }

        $this->warn('Resetting CRM for end-to-end testing...');
        ...
        $this->newLine();
        $this->info('=== Remaining System Users ===');
        $this->table(
            ['Email', 'Name', 'Role'],
        ...
        if ($report['remaining_employees'] === []) {
            $this->line('No employee records (login users are preserved; re-create employees via Employees module for E2E).');
        } else {
            $this->table(
                ['Email', 'Name'],
        ...
        $this->newLine();
        $this->info('=== Dashboard Metrics (live, post-reset) ===');
        ...
        $checks = [
            ...
            ['Ready for E2E testing', $verification['ready_for_e2e'] ? '✓' : '✗'],
        ];
        $this->table(['Check', 'Status'], $checks);

        if (! $verification['ready_for_e2e']) {
            $this->warn('Verification found issues. Review transactional_counts and orphan_records in tinker.');

            return self::FAILURE;
        }

        $this->newLine();
        $this->info('CRM is ready for fresh end-to-end testing.');
