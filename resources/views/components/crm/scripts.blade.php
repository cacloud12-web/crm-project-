@include('components.crm.asset-version')
<script src="https://unpkg.com/lucide@0.468.0/dist/umd/lucide.min.js"></script>
<script>window.__CRM_COMM_ASSETS__ = @json(asset('crm-ui/assets/communication/'));</script>
<script>window.__CRM_GOOGLE_MAPS_KEY__ = @json(config('services.google.maps_js_api_key'));</script>
<script src="{{ crm_ui_asset('crm-ui/src/constants/data.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/utils/listing-search.js') }}"></script>
{{-- Shared Reports shell + filter toolbar (must load before report-analytics.js) --}}
<script src="{{ crm_ui_asset('crm-ui/src/utils/report-filter-toolbar.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/report-shell.js') }}"></script>
{{-- Custom CRM date/time picker (replaces Flatpickr; CSS loaded in head.blade.php) --}}
<script src="{{ crm_ui_asset('crm-ui/src/components/datetime-picker.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/action-dropdown.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/instant-tooltip.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/table-pagination.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/google-maps.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/research-workspace.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/ocr-panel.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/ocr-import-page.js') }}"></script>
<script>window.__CRM_DOCUMENT_AI__ = @json(['max_file_mb' => (int) config('document-ai.max_file_mb', 10)]);</script>
<script src="{{ crm_ui_asset('crm-ui/src/api/crm.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/lead-picker.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/entity-lookup.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/demo-calendar.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/demo-calendar-data.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/demo-calendar-page.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/report-analytics.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/api/campaigns-hub.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/state-city-dropdown.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/guide-cards.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/components/template-management.js') }}"></script>
<script src="{{ crm_ui_asset('crm-ui/src/pages/pages.js') }}"></script>
@php
    $crmUser = app(\App\Services\Rbac\RbacService::class)->userPayload(auth()->user());
@endphp
<script>window.__CRM_USER__ = @json($crmUser);</script>
<script>window.CRM_FOLLOWUP_ACTIVITY_DEMO = @json((bool) config('crm_followups.demo_activity_enabled', false));</script>
<script>window.__CRM_MASTER_PIPELINE__ = @json(config('crm_master_pipeline'));</script>
<script>window.__CRM_SALES_PIPELINE__ = @json(config('crm_sales_pipeline'));</script>
<script>window.__CRM_LEAD_STATUSES__ = @json(config('crm_statuses.allowed', []));</script>
<script src="{{ crm_ui_asset('crm-ui/src/services/rbac.js') }}"></script>
<script>window.__CRM_INITIAL_PAGE__ = @json($spaPage ?? null);</script>
<script src="{{ crm_ui_asset('crm-ui/src/app.js') }}"></script>
@stack('scripts')
