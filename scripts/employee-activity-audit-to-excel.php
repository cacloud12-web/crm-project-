...
function buildWorkbook(array $data): string
{
    ...
    foreach ($data['employees'] as $emp) {
        ...
        $summaryRows[] = [
            ...
            $s['demos_total_booked'] ?? ($s['demos_scheduled_created'] ?? 0),
            $s['demos_still_scheduled'] ?? ($s['demos_still_open'] ?? 0),
            $s['demos_completed_from_booked'] ?? ($s['demos_completed'] ?? 0),
            $s['demo_completion_from_booked_pct'] ?? 0,
            $s['demos_missed_from_booked'] ?? ($s['demos_missed'] ?? 0),
            $s['followups_total'] ?? 0,
    ...
    $summaryRows[] = [
        ...
        '',
        '',
        $gt['demos_total_booked'] ?? ($gt['demos_scheduled_created'] ?? 0),
        $gt['demos_still_scheduled'] ?? 0,
        $gt['demos_completed_from_booked'] ?? ($gt['demos_completed'] ?? 0),
        '',
        $gt['demos_missed_from_booked'] ?? 0,
